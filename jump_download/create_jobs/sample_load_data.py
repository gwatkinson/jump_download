...
def sample_from_load_data(
    ...
    if not negative_controls:
        print("Skipping negative controls")
    else:
...
def create_output_dir_column(
    ...
    source_to_disk_series = pd.Series(source_to_dict, name=col_name)
    return load_data.merge(source_to_disk_series, left_on="Metadata_Source", right_index=True)
...
def create_filter_column(
    ...
    output_dir_func: Callable[[pd.DataFrame], pd.DataFrame],
    ...
    load_data = output_dir_func(load_data)
...
def create_submission_csv(job_output_dir):
    jobs_csv = glob(os.path.join(job_output_dir, "ids", "*.parquet"))
    submission_df_path = os.path.join(job_output_dir, "submission.csv")
    job_ids = [Path(p).stem.split(".")[0] for p in jobs_csv if "__dropped" not in p]

    submission_df = pd.DataFrame({"jobs_csv": jobs_csv}).sample(frac=1, random_state=42)
...
def sample_load_data(cfg: DictConfig):
    ...
    job_output_dir = cfg.output_dirs.job_output_dir
    plate_types_to_keep = cfg.filters.plate_types_to_keep
    source_dict = instantiate(cfg.output_dirs.source_to_disk, _convert_="all")
    ...
    if "COMPOUND" not in plate_types_to_keep:
        print("\n=== Skipping COMPOUND sampling...")
    else:
        ...
        samples_dict.append(compound_samples)

    if "ORF" not in plate_types_to_keep:
        print("\n=== Skipping ORF sampling...")
    else:
        ...
        samples_dict.append(orf_samples)

    if "CRISPR" not in plate_types_to_keep:
        print("\n=== Skipping CRISPR sampling...")
    else:
        ...
        samples_dict.append(crispr_samples)

    if "TARGET1" not in plate_types_to_keep or "TARGET2" not in plate_types_to_keep:
        print("\n=== Skipping TARGET sampling...")
    else:
        ...
        samples_dict.append(target_samples)

    print("\n=== Creating filter column and job ids...")
    print(isinstance(source_dict, dict))
    output_dir_func = partial(
        create_output_dir_column, source_to_dict=source_dict, col_name="output_dir"
    )
    job_df = create_filter_column(
        load_data=load_data,
        samples_dict=samples_dict,
        output_dir_func=output_dir_func,
    ...
    print("\n=== Splitting job dataframe into parquet files...")
    split = {k: v for k, v in job_df.groupby("job_id")}
    for k, v in tqdm(split.items()):
        if "__dropped" not in str(k):
            output = os.path.join(job_output_dir, "ids", f"{k}.parquet")
            v.to_parquet(
                output,
                engine="pyarrow",
                compression="snappy",
